...
export default function Politics() {
  const navigation = useNavigation();
    ...
    useEffect(() => {
        axios
        .get("http://newsserver.abhiyanta.co/api/categorywise_articles/politics")
        .then((response) => {
    ...
    return (
        ...
        <SafeAreaView style={styles.container}>
          <ScrollView>
                        <View style={{flex:1}}>
                            <Text style={styles.trendinghead}> Politics Categories</Text>
                        </View>
                    {
            sportList.map((item)=>
            <TouchableHighlight  key={item.key} underlayColor={'transparent'} onPress={()=> navigation.navigate("PoliticsDetails",item)}>
    <Card>
